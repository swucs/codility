...
/**
 * DNA 서열은 A , C , G 및 T 문자로 구성된 문자열로 나타낼 수 있으며 , 이는 서열의 연속적인 뉴클레오티드 유형에 해당합니다. 각 뉴클레오티드에는 정수인 영향 계수 가 있습니다. A , C , G 및 T 유형 의 뉴클레오티드는 각각 1, 2, 3 및 4의 영향 계수를 갖습니다. 다음과 같은 형식의 몇 가지 질문에 답할 것입니다. 주어진 DNA 서열의 특정 부분에 포함된 뉴클레오티드의 최소 영향 인자는 무엇입니까?
 *
 * DNA 시퀀스는 N개의 문자로 구성된 비어 있지 않은 문자열 S = S[0]S[1]...S[N-1] 로 제공됩니다. 비어 있지 않은 배열 P 및 Q에 제공되는 M 쿼리가 있으며 각각 M 정수로 구성되어 있습니다. K 번째 쿼리(0 ≤ K < M)에서는 위치 P[K]와 Q[K](포함) 사이의 DNA 시퀀스에 포함된 뉴클레오티드의 최소 영향 계수를 찾아야 합니다.
 *
