...
/**
 * N개의 정수로 구성된 비어 있지 않은 배열 A가 제공됩니다. 0 ≤ P < Q < N과 같은 한 쌍의 정수(P, Q) 를 배열 A 의 슬라이스 라고 합니다( 슬라이스 에 두 개 이상의 요소가 포함됨). 평균 슬라이스 (P, Q)의이 A [P] + A의 합 [P + 1] + ... + A [Q] 슬라이스의 길이로 나눈 값. 정확히 말하면 평균은 (A[P] + A[P + 1] + ... + A[Q]) / (Q − P + 1)과 같습니다.
 *
 * 예를 들어, 다음과 같은 배열 A:
 *
 *     A[0] = 4
